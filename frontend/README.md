...
- [x] Как итоге статья удаляется из БД и из стейта в redux
...
- [x] Принцип такой - при переходе на страницу с редактированием поста - мы переходим на роут с id поста типа `http://localhost:3000/posts/:id/edit`. Но рендерим компонент создания поста.
- [x] `useParams()` получем `id` в url. Если `id` есть - поля статьи будут заполнены данными статья. Если `id` нет - поля пустые и это обычное создание статьи.
- [x] Исходя из данных выше - делаем запрос либо `post` для создания, либо `patch` для изменения.
- [x] + Кнопка сохранить / опубликовать статью будет подстраиваться по ситуации исходя из переменной `const isEditing = Boolean(id)`, которая меняется в зависимости от наличия `id`.
...
<h3>+ Запрос и обновление статьи</h3>

- [x] Как и в прошлом пункте, посколько компонент создания и редактирования один - нужно понимать какой запрос отсылать. `post` или `patch`. Для этого мы применяем вышеупомянутый `const isEditing = Boolean(id)`. Те если мы получили id то это редактирование, а если id нет - это создание.
- [x] Так же адаптируем отправку статьи на сервер под два случая. Под создание и редактирование. В остальном создание формы и тд такое же. 

[КОМПОНЕНТ СТРАНИЦЫ СОЗДАНИЯ / ИЗМЕНЕНИЯ СТАТЬИ](https://github.com/acidshotgun/full-stack-blog/blob/master/frontend/src/pages/AddPost/index.jsx)
