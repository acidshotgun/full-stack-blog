...
- [x] Этот `middleware` будет использоваться в тех запроса, которые смогут делать только авторизированные пользователи. Напирмер смотреть могут все, а создавать только зарегистрированные. 

```javascript
import jwt from "jsonwebtoken";

// Тут это как безымянная ф-я но можно и с именем.
...
// получение юзера
//  1) Роут
//  2) Проверка авторизации (middleware)
//  3) Контроллер на авторизацию.
...
```javascript
//  ПОЛУЧЕНИЕ ПОЛЬЗОВАТЕЛЯ (контроллер)
...
- [x] Валидация для создания / изменения постов. Это `middleware`, который который будет проверять посты на правильность заполнения.
...
// Роут создания поста
//  1) Роут
//  2) Првоерка авторизации (middleware)
//  3) Валидация (middleware)
//  4) Ошибки валидации (middleware)
//  5) Контроллер создания поста
router.post("/posts", checkAuths, postCreateValidation, handleValidationErrors,PostContoller.create);
```

```javascript
// Создание поста (контроллер)
...
```javascript
//  1) Роут
//  2) Контроллер полуучения постов
router.get("/posts", PostContoller.getAll);
```

```javascript
// Получить все посты (контроллер)
...
```javascript
//  1) Роут с динамическим id поста
//  2) Контроллер поста
router.get("/posts/:id", PostContoller.getOne);
```

```javascript
// Получение одного поста (контроллер)
...
```javascript
// Роут обновления поста
//  1) Роут обновления с динамическим id поста
//  2) Проверка авторизации (middleware)
//  3) Валидация обновления поста (middleware)
//  4) ошибки валидации (middleware)
//  5) Контроллер обновления поста
router.patch("/posts/:id", checkAuths, postCreateValidation, handleValidationErrors, PostContoller.update);
```

```javascript
// Обновление поста (контроллер)
...
- [x] ПРИМ. ТУТ МЫ БУДЕМ ХРАНИТЬ КАРТИНКИ НА СЕРВЕРЕ. НО НАДО В ОБЛАКЕ - РАЗБИРАЮСЬ.

- [x] В загрезке медиа-файлов поможет библиотека `multer`, которая предназначена для обработки файлов, загружаемых через формы.
- [x] Цель:

+ Multer предназначен для обработки файлов, которые клиенты загружают на веб-сервер через формы (например, изображения, видео, документы).
+ Он обрабатывает множество файлов, позволяя вам получить доступ к данным о каждом загруженном файле.

- [x] Основные возможности Multer:

+ Поддерживает загрузку файлов в память (buffer) или на диск.
+ Позволяет настраивать параметры загрузки, такие как максимальный размер файла, допустимые MIME-типы и другие ограничения.
+ Прост в использовании и интегрируется легко с Express.js.

<br>
<hr>

<h3>+ Загрузка картинок (Multer)</h3>

- [x] Первым делом нужно создать хранилище. И для этого есть след. методы:

+ `diskStorage()` - файлы будут хранится на сервере на специальном диске.
+ `memoryStorage()` - файлы хранятся в RAM на сервере какое то время. Подходит, когда с сервера нужно отправить файл в облако.
...
const storage = multer.diskStorage({
  // Путь
  // ничего не передаем => указываем _, __
  // null - ошибок нет, "uploads" имя папки
  destination(_, __, cb) {
    cb(null, "uploads");
  },
  // Имя файла (тут можно изменить имя файла)
  // возвращаем:
  //  null - ошибок нет
  //  file.originalname - оригинальное имя картинки
  filename(_, file, cb) {
...
- [x] Т.к. изображения храним на сервере - указываем, что мы должны получать статические файлы при их запроса. (а не из какого-то облака например)
...
- [x] Запрос на добавление картинки
