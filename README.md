...
<h2>ПОЛУЧЕНИЕ ИНФОРМАЦИИ О ПРОФИЛЕ</h2>

- [x] Задача состоит в том, чтобы написать `**middleware**`, который будет проверять по токену (который мы везде шифровали) - имеет ли пользователь доступ (авторизован ли он). Так же имеет ли пользователь по расшифрованному токену выполнять какие - либо действия. (Например получать информацию о себе)
- [x] Так же имеет ли он токен и имеет ли доступ к этому запросу.
